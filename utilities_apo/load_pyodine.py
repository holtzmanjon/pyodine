...
from utilities_apo import conf
...
class ObservationWrapper(components.Observation):
    ...
    def __init__(self, filename, instrument=None, star=None):
        ...
        #          obstime=t,location=EarthLocation.of_site('APO'))
        #ltt = t.light_travel_time(sc,location=EarthLocation.of_site('APO'))
        #barytime = t.tdb + ltt + TimeDelta(exptime/2.*u.second)
        #self.bary_date = barytime.jd
        #self.bary_vel_corr = barycorr.to(u.km/u.s).value * 1000. 
...
def load_file(filename) -> components.Observation:
    ...
    try:
        ...
        if ext == '.fits':
            ...
            header = h[0].header
            header['PROGRAM'] = 'dummy'
            # Prepare data
            flux = h['FLUX'].data[:,12:][:,::-1]
            cont = h['RESPONSE'].data[:,12:][:,::-1]
            wave = h['WAVE'].data[:,12:][:,::-1]
            #weight = None
...
def get_instrument(header) -> components.Instrument:
    ...
    if 'TELESCOP' in header:
        ...
        elif 'Node 4' in header['TELESCOP'] and 'Spectrograph' in header['INSTRUME'] :
            return conf.my_instruments['apo']
        elif 'APO' in header['INSTRUME'] :
            return conf.my_instruments['apo']
    else:
